"""
Automatic Modulation Classification on RadioML 2016.10b
=========================================================

CNN + Bi-LSTM + Additive Attention model for AMC on the RadioML 2016.10b
dataset, with a custom exponential learning-rate schedule. See README.md
...
# ----------------------------------------------------------------------------
RANDOM_STATE = 2016
FILE_PATH = '/content/drive/MyDrive/RML2016.10b/RML2016.10b.dat'
...
def build_improved_model(num_classes, input_shape=(2, 128), initial_lr=0.001):
...
if __name__ == '__main__':
    ...
    initial_lr = 0.001
    model = build_improved_model(len(classes), input_shape=(2, 128), initial_lr=initial_lr)
    lr_scheduler = CustomLearningRateScheduler(initial_lr=initial_lr, decay_factor=0.95, min_lr=1e-6)
    ...
    plt.subplot(1, 3, 3)
    plt.plot([initial_lr * (0.95 ** i) for i in range(len(history.history['loss']))], label='Learning Rate')
    plt.legend()
